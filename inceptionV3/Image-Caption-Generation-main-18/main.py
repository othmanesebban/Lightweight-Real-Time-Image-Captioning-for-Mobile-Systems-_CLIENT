...
parser.add_argument(
    "--inception_path",
    type=str,
    help="A valid path to inception_v3.pb",
    default="ConvNets/inception_v3.pb")
parser.add_argument(
    "--saveencoder",
...
if config.mode == "train":
    ...
    ########################### othmane sebban ##################################
    features = np.array([feat.astype(np.float32) for feat in features])
...
#################################### othmane sebban ##############################################
    ...
    if os.path.exists(args.validation_data):
        ...
        with open("captions_generations.txt") as f:
           data = f.readlines()
        #with open("Dataset/image_info_test2017.json", 'r') as f:
            #data = json.load(f)
        ...
        # Construisez votre modèle Caption_Generator avec la configuration "config"
        model = Caption_Generator(config)
