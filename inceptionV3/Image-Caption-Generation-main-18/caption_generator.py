...
class Caption_Generator():
    ...
    def Words_to_IDs(self, wtoidx, caption_batch):
        max_len = 18  # Longueur maximale pour les légendes
        for i, caption in enumerate(caption_batch):
            ...
            for word in words:
                if word in wtoidx:
                    ...
                    cap.append(wtoidx["<UNK>"])  # Remplacer les mots inconnus par "<UNK>"
            # Pad ou tronquer la liste de mots pour atteindre la longueur maximale
            if len(cap) < max_len:
                cap += [wtoidx["<PAD>"]] * (max_len - len(cap))
            else:
                cap = cap[:max_len]
    ...
    def generate_mask(self, ID_batch, wtoidx):
        ...
        ID_batch_new = np.zeros((ID_batch.shape[0], 18))  # Utilisation de la nouvelle longueur (18 mots)
        nonpadded_list = list(nonpadded)
        for ind, row in enumerate(ID_batch_new):
    ...
    def train(self, loss, inp_dict):
        ...
        # Add code here to calculate and append validation_losses during validation
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.title('Loss Plot')
        plt.legend()
            ...
            with open("model/Decoder/Generated_Captions.txt", 'w') as f:
                ...
                with tf.Session() as sess:
                    ...
                    for i, feat in enumerate(features):
                        ...
                        # ...
                        feat = np.reshape(feat, newshape=(1, 2048))
