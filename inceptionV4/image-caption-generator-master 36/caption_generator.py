...
class Caption_Generator():
    ...
    def Words_to_IDs(self, wtoidx, caption_batch):
        max_caption_length = self.num_timesteps
        padded_captions = []
        for caption in caption_batch:
            words = caption.split()
            if len(words) < max_caption_length:
                # Pad the caption with "<PAD>" tokens to match the expected length
                words += ["<PAD>"] * (max_caption_length - len(words))
            # Truncate the caption if it's longer than expected
            words = words[:max_caption_length]
            cap = [wtoidx[word] if word in wtoidx else wtoidx["<UNK>"] for word in words]
            padded_captions.append(cap)
        return np.vstack(padded_captions)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.current_epoch, self.nb_epochs):
                ...
                for batch_idx in range(self.num_batch):
                    ...
                    # Create a mask based on the actual caption length
                    caption_lengths = np.count_nonzero(batch_Ids, axis=1)
                    batch_mask = np.zeros((self.batch_size, self.num_timesteps))
                    for i, length in enumerate(caption_lengths):
                        batch_mask[i, :length] = 1
                    run = [global_step, optimizer, self.loss, summary_op]
                    feed_dict = self.create_feed_dict(batch_Ids, batch_features, batch_mask)
    ...
    def batch_decoder(self, filenames, features):
        ...
        print("Début de la boucle de décodage")
        try:
            with open("model/Decoder/Generated_Captions.txt", 'w') as f:
                print("File opened for writing:", "model/Decoder/Generated_Captions.txt")
                # Your caption generation and writing logic here
                # Ajoutez un message d'impression pour vérifier que le fichier est ouvert avec succès
                print("Fichier ouvert pour écriture")
                with tf.Session() as sess:
                    init = tf.global_variables_initializer()
                    sess.run(init)
                    saver.restore(sess, ckpt_file)
                    for i, feat in enumerate(features):
                        print("Processing feature", i, "out of", features.shape[0])
                        if i < len(filenames):
                            # ...
                            ...
                            # Ajoutez des messages d'impression pour vérifier les résultats
                            print("Résultats de la génération de légendes :")
                            print("ID des légendes :", caption_IDs)
                            sentence = " ".join(
                                self.IDs_to_Words(
                                    self.idxtow, caption_IDs))
                            sentence = sentence.split("</S>")[0]
                            # Ajoutez un message d'impression pour afficher les légendes générées
                            print("Légende générée :", sentence)
                            if i % 1000 == 0:
                                print("Progress", i, "out of", features.shape[0])
                            f.write(filenames[i] + "\t" + sentence + "\n")
                            print("Caption written to the file")
                        else:
                            print("Index out of bounds:", i)
        except Exception as e:
              print("Error while opening/writing the file:", str(e))
